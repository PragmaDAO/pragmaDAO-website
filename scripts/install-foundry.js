...
async function installFoundry() {
    ...
    // Check if we're in Docker container (where Foundry is pre-installed)
    if (process.env.DOCKER_IN_DOCKER === 'false' || fs.existsSync('/.dockerenv')) {
        console.log('🐳 Docker container detected, skipping Foundry installation (pre-installed)');
        return;
    }
